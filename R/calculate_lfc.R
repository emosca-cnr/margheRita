#' Calculate the log fold change between two classes of samples
#' 
#' The log2 fold changes are calculated using the mean values of the two classes
#' 
#' @export
#' @param  mRList mRList object
#' @param contrast_samples character vector with two elements that indicate the two classes of samples
#' @return mRList object with fold changes in mRList$metab_ann 

calculate_lfc <- function(mRList, contrast_samples){

    log_data <- mRList$metab_ann[, grep(paste(contrast_samples[1], "_mean", "|", contrast_samples[2], "_mean", sep = ""), colnames(mRList$metab_ann))]
    ...
    colnames(mRList$metab_ann)[ncol(mRList$metab_ann)] <- names(log_data)[3]
