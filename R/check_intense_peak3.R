...
#' @export
#'
check_intense_peak3 = function(RT_mass=NULL, RI_lib=NULL , RI_sample=NULL, reference=NULL, lib_peaks_data=NULL, ppm_err=10, intensity=30, mode=c("POS", "NEG")) {
  ...
  #z cycles through the precursor library entries
  ...
  for(v in 1:length(ans)){
    peaks= toString(ans[[v]]$ID_peaks)
    ans[[v]]$all_peaks = peaks
    ans[[v]] = ans[[v]][!duplicated(ans[[v]]$Feature_ID),]
  }
  ...
  if(length(ans)>0){
    ...
    ans <- ans[, c("Feature_ID", "ID", "ID_peaks", "CAS", "PubChemCID", "Name", "rt", "RT_err", "RT_flag", "mz", "ppm_error", "mass_status", "mass_flag", "Name_peaks", "peaks_found_ppm_RI", "Collision_energy","all_peaks")]
  }else{
