...
metabolite_family = function(feature_data = NULL , reference = NULL, rt_err_thr= 2,
  ...
  RT_mass=NULL, RI_lib=NULL , RI_sample=NULL,  lib_peaks_cas=NULL, n_peaks=1, acceptable_PPM_err = 10, mode=c("POS", "NEG")) {
  ...
  ###merge RT & mass
  RT_mass = check_RT_mass (RT , mass, reference= data$lib_precursors)
...
for(z in 1:length(RT_mass)){
  ...
  if(length(z_peaks_all)>0){
    ...
    for(zzzz in 1:length(z_peaks_all)){
      ...
      #RT_mass[[z]]$ID_peaks <- RT_mass[[z]]$ID_peaks= names(z_peaks_all)[zzzz]

      RT_mass[[z]] <- data.frame(Feature_ID= RT_mass[[z]]$Feature_ID, RT_err = RT_mass[[z]]$RT_err, ppm_error=RT_mass[[z]]$ppm_error,peaks_found_ppm_RI = RT_mass[[z]]$peaks_found_ppm_RI , peaks_found_flag= RT_mass[[z]]$peaks_found_flag, ID_peaks=names(z_peaks_all)[zzzz])
