...
peak_matching_onlyMSMS <- function(library_list=NULL, RT_mass=NULL, RI_sample=NULL, ppm_err=10, intensity=30, RI_diff_type=c("rel", "abs")) {
    
  
  #a novel data to not touch the input
  if(is.null(RT_mass)){
    cat("No RT_mass, checking all MS/MS from library_list$lib_peaks againts all RI_sample\n")
    
    #create an RT_mass with all Feature_ID for all library elment
    RT_mass <- setNames(vector("list", nrow(library_list$lib_precursor)), library_list$lib_precursor$ID)
  ...
  lib_peaks_data <- lib_peaks_data[match(names(library_list$lib_peaks), lib_peaks_data$ID), ]
  ...
  for(z in 1:length(RT_mass)){
    ...
    CAS_z <- library_list$lib_precursor$CAS[library_list$lib_precursor$ID == names(RT_mass)[z]]
    
    ### mz of the library element (precursor)
    mz_precursor <- library_list$lib_precursor$mz[library_list$lib_precursor$ID == names(RT_mass)[z]]
    
    ##the list of the MS/MS spectra for the library element (it can be not unique)
    #z_peaks_all <-  library_list$lib_peaks[lib_peaks_data$CAS == CAS_z & lib_peaks_data$Collision_energy == mode ] #depracated because the library is already in the desired mode
    z_peaks_all <-  library_list$lib_peaks[lib_peaks_data$CAS == CAS_z]
    ...
    ### merge RT_mass of the library element with ans by Feature_ID, add library_list$lib_precursor info
    if(!is.null(ans[[z]])){
