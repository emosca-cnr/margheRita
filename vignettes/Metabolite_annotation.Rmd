...
margheRita provides a library contains about 600 compounds in five possible column of HILIC, LipC8, pZIC, RPLong, RPShort in positive and negative mode for metabolite identification of unknown metabolite. to generate the library two datasets are provided, one related to precursor contain 16 types of information. Name, Elemental composition, CAS, COL, Unique.Name_lib, rt, mz in NEU, POS and NEG mode, SMILES, KEGG.ID or CHEMSPIDER.ID, HMDB/YMDBID, METLIN.ID, PubChemCID, CHEBI and ID. 
the other one relate to fragments contains 9 features of ID, Name, MW, CAS, Formula, Precursor MZ, Collision energy, Number of the peaks and peaks.
...
```{r, eval=FALSE}
select_library(column=c("HILIC", "LipC8", "pZIC", "RPLong", "RPShort"), mode=c("POS", "NEG"), accept_RI=10)
```
...
sample data set can be in Excel or ... format. while mass spectroscopy provides variety of information, the requirements for metabolite identifications are Retention time and m/z and spectra of the fragments. 

```{r, eval=FALSE}
select_sample(mRList = NULL, n_column = c(1, 2, 3, 5), accept_RI = 10)
```


# generate sample for metabolite identification 
mass spectroscopy provides variety of information but for metabolite identification its only require Retention time and m/z and spectra of the known metabolite library and unknown sample to compare them together. 
